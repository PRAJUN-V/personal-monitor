        ...
        <div className="animate-in space-y-8">
            ...
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    ...
                    <button onClick={() => setShowTransForm(true)} className="bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg transition flex items-center gap-2">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>New Payment
                    </button>
                </div>
                    ...
                    {showSourceForm ? (
                        <form onSubmit={handleSourceSubmit} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-xs font-bold text-indigo-300 block mb-1">Name</label><input type="text" required className="w-full bg-indigo-800 rounded-xl p-3 text-white" value={sourceForm.name} onChange={e => setSourceForm({...sourceForm, name: e.target.value})} /></div>
                                <div><label className="text-xs font-bold text-indigo-300 block mb-1">Initial Balance</label><input type="number" required className="w-full bg-indigo-800 rounded-xl p-3 text-white" value={sourceForm.balance} onChange={e => setSourceForm({...sourceForm, balance: e.target.value})} /></div>
                            </div>
                            <button type="submit" className="w-full bg-white text-indigo-900 font-bold py-3 rounded-xl">Create Source</button>
                        </form>
                    ) : (
                        <form onSubmit={handleTransSubmit} className="space-y-4">
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                <div className="col-span-2 md:col-span-1"><label className="text-xs font-bold text-indigo-300 block mb-1">Source</label><select required className="w-full bg-indigo-800 rounded-xl p-3 text-white" value={transForm.source_id} onChange={e => setTransForm({...transForm, source_id: e.target.value})}><option value="">Select Account</option>{sources.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
