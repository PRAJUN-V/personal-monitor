function HealthMonitor({ records, page, onPageChange, onSave, onEdit, onDelete, isSaving, isFetching }) {
    const [showForm, setShowForm] = React.useState(false);
    const [editingId, setEditingId] = React.useState(null);
    const [form, setForm] = React.useState({ date: new Date().toISOString().split('T')[0], height: '', weight: '', bp_systolic: '', bp_diastolic: '' });

    const handleFormSubmit = async (e) => {
        e.preventDefault();
        const success = await onSave(form, editingId);
        if (success) { setShowForm(false); setEditingId(null); setForm({ date: new Date().toISOString().split('T')[0], height: '', weight: '', bp_systolic: '', bp_diastolic: '' }); }
    };

    const startEdit = (record) => {
        setEditingId(record.id);
        setForm({ date: record.date, height: record.height, weight: record.weight, bp_systolic: record.bp_systolic || '', bp_diastolic: record.bp_diastolic || '' });
        setShowForm(true);
    ...
    // Skeleton Component
    const Skeleton = () => (
        <div className="animate-pulse space-y-4 p-5 bg-white">
            <div className="flex justify-between"><div className="h-4 bg-slate-200 rounded w-1/3"></div><div className="h-6 bg-slate-200 rounded w-1/4"></div></div>
            <div className="grid grid-cols-3 gap-4"><div className="h-8 bg-slate-100 rounded"></div><div className="h-8 bg-slate-100 rounded"></div><div className="h-8 bg-slate-100 rounded"></div></div>
        </div>
    );
                ...
                <React.Fragment>
                    ...
                    {records.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                                <p className="text-xs font-medium text-slate-400 uppercase mb-1">Latest BMI</p>
                                <div className="flex items-end gap-2"><span className="text-3xl font-bold">{records[0].bmi}</span><span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getBMICategoryColor(records[0].category)}`}>{records[0].category}</span></div>
                            </div>
                            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                                <p className="text-xs font-medium text-slate-400 uppercase mb-1">Latest Weight</p>
                                <div className="flex items-end gap-2"><span className="text-3xl font-bold">{records[0].weight}</span><span className="text-slate-400 text-sm mb-1">kg</span></div>
                            </div>
                            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
                                <p className="text-xs font-medium text-slate-400 uppercase mb-1">Goal Status</p>
                                <div className="flex items-center gap-2 mt-1"><span className="relative flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-500"></span></span><span className="text-sm font-semibold text-emerald-600 italic">{records[0].weight_diff_to_normal === 0 ? "Normal" : `${records[0].weight_diff_to_normal > 0 ? "Lose" : "Gain"} ${Math.abs(records[0].weight_diff_to_normal)}kg`}</span></div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
                        <div className="p-6 border-b flex justify-between items-center bg-slate-50/50">
                            ...
                            <button onClick={() => { setShowForm(!showForm); if (showForm) setEditingId(null); }} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg">{showForm ? 'Close' : 'Add Entry'}</button>
                        </div>
                                ...
                                {records.map(r => (
                                    <div key={r.id} className="p-5 bg-white space-y-4">
                                        ...
                                        <div className="grid grid-cols-3 text-xs">
                                            <div><p className="text-slate-400 font-bold uppercase mb-1">Ht</p><p className="font-bold">{r.height}cm</p></div>
                                            <div><p className="text-slate-400 font-bold uppercase mb-1">Wt</p><p className="font-bold">{r.weight}kg</p></div>
                                            <div><p className="text-slate-400 font-bold uppercase mb-1">BMI</p><p className="font-bold text-indigo-600">{r.bmi}</p></div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                    ...
                                    {records.map(r => (
                                        <tr key={r.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="p-4 text-right">
                                                <button onClick={() => startEdit(r)} className="text-slate-300 hover:text-indigo-600 mr-4 font-bold text-[10px]">Edit</button>
                                                <button onClick={() => onDelete(r.id)} className="text-slate-300 hover:text-rose-600 font-bold text-[10px]">Delete</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                        ...
                        <div className="p-4 border-t flex justify-between text-xs bg-slate-50">
                            <button disabled={page === 0 || isFetching} onClick={() => onPageChange(page - 1)} className="px-4 py-2 border rounded-xl bg-white font-bold uppercase text-[10px] disabled:opacity-30">Prev</button>
                            <button disabled={records.length < 5 || isFetching} onClick={() => onPageChange(page + 1)} className="px-4 py-2 border rounded-xl bg-white font-bold uppercase text-[10px] disabled:opacity-30">Next</button>
                        </div>
                    </div>
                </React.Fragment>
            )}
